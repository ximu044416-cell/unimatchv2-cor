...
def get_cosine_threshold(epoch, total_epochs, max_thresh=0.85, min_thresh=0.55, start_decay_epoch=150):
    ...
    current_thresh = min_thresh + (max_thresh - min_thresh) * cosine_decay
...
def get_dynamic_tversky_alpha_beta(epoch, start_beta=0.5, target_beta=0.7, rampup_epochs=50):
    if epoch >= rampup_epochs:
        eff_beta = target_beta
    else:
        eff_beta = start_beta + (target_beta - start_beta) * (epoch / rampup_epochs)
    eff_alpha = 1.0 - eff_beta
    return eff_alpha, eff_beta
...
def apply_cutmix_with_aux(data, target_main, target_aux, probability=0.5, beta=1.0):
    """
    让图像、主标签(mask) 和 辅助标签(neg_prior) 共享同一套 CutMix 参数
    """
    if np.random.rand() > probability:
...
def get_llrd_params(model, lr_backbone, lr_head, weight_decay, decay_rate=0.90):
    ...
    if len(norm_params) > 0:
        groups.append({'params': norm_params, 'lr': lr_backbone, 'weight_decay': weight_decay})
...
logging.basicConfig(
    filename=os.path.join(config.OUTPUT_DIR, 'train_log.txt'),
    level=logging.INFO,
...
def train():
    ...
    logging.info("🚀 启动 RUN8（strict 40-case hold-out test; test folder is excluded from training）")
    ...
    for epoch in range(config.TOTAL_EPOCHS):
        ...
        if epoch < 150:
            ...
            if current_stage == 1:
                logging.info(f"🔓 [Epoch {epoch}] 释放 Backbone，进入解冻阶段")
                for param in model.encoder.parameters():
                ...
                logging.info("♻️ EarlyStopping 已清零")
                current_stage = 2
            ...
            if 150 <= epoch < 165:
                ...
                if epoch == 164:
                    ...
                    scheduler = optim.lr_scheduler.CosineAnnealingLR(
                        optimizer,
                        T_max=(config.TOTAL_EPOCHS - 165),
                        eta_min=min_lr_backbone
                    )
                    logging.info(f"🚀 [Epoch {epoch}] 解冻预热结束，切换到 CosineAnnealingLR")
        ...
        current_ft_alpha, current_ft_beta = 0.3, 0.7
        ...
        for step, batch_l in enumerate(pbar):
            ...
            # labeled 分支：CutMix 同步作用在 image / mask / neg_prior
            ...
            metrics_meter['neg_sup'] += loss_neg_sup.item()
            metrics_meter['neg_unsup'] += loss_neg_unsup.item()
        ...
        tb_writer.add_scalar('Sys/FocalTversky_Alpha', current_ft_alpha, epoch)
        tb_writer.add_scalar('Sys/FocalTversky_Beta', current_ft_beta, epoch)
        ...
        logging.info(
            ...
            f"Best(Th={best_th_str}) Dice {best['dice']:.4f}, Rec {best['recall']:.4f}, "
            f"Prec {best['precision']:.4f}, AUC {best['auc']:.4f} | "
            f"Patience {patience_status}"
            f" | FT(alpha={current_ft_alpha:.3f}, beta={current_ft_beta:.3f})"
        )

        if epoch >= SWA_START_EPOCH:
            swa_model.update_parameters(model)
            logging.info(f"🔄 SWA 权重池已捕获 (Epoch {epoch})")
            ...
            if epoch == config.TOTAL_EPOCHS - 1:
                ...
                logging.info(
                    f"🏆 SWA 终极成绩 (Th=0.50): Dice {swa_val_metrics['dice']:.4f}, "
                    f"Rec {swa_val_metrics['recall']:.4f}, Prec {swa_val_metrics['precision']:.4f}"
                )

                torch.save(
                    swa_model.module.state_dict(),
                    os.path.join(config.OUTPUT_DIR, "best_model_SWA.pth")
                )
        ...
        if early_stopping.early_stop:
            ...
            if epoch >= SWA_START_EPOCH:
                ...
                update_bn(dl_l, swa_model, device=device)
                swa_val_metrics = validate_metrics_full(swa_model.module, dl_val, device, thresh=0.50)
                logging.info(f"🏆 SWA 终极成绩 (Th=0.50): Dice {swa_val_metrics['dice']:.4f}")
                torch.save(
                    swa_model.module.state_dict(),
                    os.path.join(config.OUTPUT_DIR, "best_model_SWA.pth")
                )
