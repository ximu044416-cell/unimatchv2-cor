...
def apply_cutmix(data, target_main, probability=0.5, beta=1.0):
    """
    prior_no_penalty 消融中仍然对 4 通道输入做 CutMix，
    但不再需要同步处理 negative-prior penalty map。
    """
...
def train():
    ...
    logging.info("=" * 90)
    logging.info("🚀 prior_no_penalty ablation started")
    ...
    for epoch in range(TOTAL_EPOCHS):
        ...
        pbar = tqdm(dl_l, total=len(dl_l), desc=f"PriorNoPenalty Ep {epoch + 1}/{TOTAL_EPOCHS}")
        optimizer.zero_grad()

        for step, batch_l in enumerate(pbar):
            # Dataset still returns neg_l, but this ablation intentionally ignores it.
            ...
            # Dataset still returns neg_u, but this ablation intentionally ignores it.
    ...
    tb_writer.close()
    logging.info("✅ prior_no_penalty ablation finished.")
