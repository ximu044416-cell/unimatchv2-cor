...
def extract_test_dual_track_radiomics():
    ...
    TEST_DIR = Path(r"F:\downstreamtasks\radiomics\test")
    YAML_PATH = Path(r"radiomics_features.yaml")

    # 最终生成的两份特征表
    OUTPUT_GT_CSV = Path(r"F:\downstreamtasks\radiomics\Test_GT_Features.csv")
    OUTPUT_PRED_CSV = Path(r"F:\downstreamtasks\radiomics\Test_Pred_Features.csv")
