...
def run_ultimate_blind_test():
    ...
    WORK_DIR = r"F:\downstreamtasks\radiomics"
    ...
    # 🔥 核心升级 1：自动读取 Step 4/5 冻结的 LASSO 权重
    ...
    # 极简全明星阵容 (与你 Step 4/5 选出的最终模型完美一致)
    ...
    # 🔥 核心升级 2：直接读取 Step 5 锁定的最佳临床 Cutoff
    ...
    # =========================================================================
    # 🚑 导师紧急抢救包：修复“空掩码患者”的异常极值灾难
    # 1. 寻找被我们强制塞入 0.0 的病人（探测器：只要第一个特征是 0.0 就是空掩码）
    probe_feature = selected_features[0]
    empty_mask_idx = df_test_merge_pred[probe_feature] == 0.0

    # 2. 既然 AI 认为他们没水肿，他们的 Rad_score 应该代表“极其健康”
    # 取训练集中真正健康患者 (Label=0) 的最低 Rad_score 作为惩罚分
    healthy_min_radscore = df_train.loc[df_train['Label'] == 0, 'Rad_score'].min()

    # 3. 强行纠偏！将这些漏诊病人的风险分直接压到底
    ...
    print("\n📊 基于锁定阈值 (Cutoff={:.4f}) 的硬核指标：".format(locked_cutoff))
    ...
    print("🎉 顶刊图表三连击生成完毕！大考顺利结束！去写论文吧！")
