...
def hosmer_lemeshow_test(y_true, y_prob, g=10):
    ...
    返回:
        hl_stat, p_value, n_groups, grouped_table
    """
    df = pd.DataFrame({
...
def plot_final_calibration(y_true, gt_prob, ai_prob, out_path, hl_gt_p=None, hl_ai_p=None):
    ...
    if hl_gt_p is not None and hl_ai_p is not None and not (np.isnan(hl_gt_p) or np.isnan(hl_ai_p)):
        text = f"HL test\nGT: P = {hl_gt_p:.4f}\nAI: P = {hl_ai_p:.4f}"
        plt.text(
            0.64, 0.18, text,
            transform=plt.gca().transAxes,
...
# =========================================================
def main():
    print("🚀 启动 RUN7 Step4（radiomics_2 测试端正式评估 + HL test）")
    ...
    # GT HL
    hl_gt_stat_full, hl_gt_p_full, hl_gt_groups_full, hl_gt_table_full = hosmer_lemeshow_test(
        y_true_full, gt_prob_full, g=10
    )
    if len(hl_gt_table_full) > 0:
        hl_gt_table_full.to_csv(
            os.path.join(EVAL_DIR, "GT_HosmerLemeshow_Table.csv"),
            index=False,
            encoding='utf-8-sig'
        )
    ...
    print(f"   HL  = {hl_gt_stat_full:.3f} | P = {hl_gt_p_full:.4f} | groups = {hl_gt_groups_full}")
    ...
    for thr in THRESHOLDS:
        ...
        for mode_name, mode_cfg in mode_dict.items():
            ...
            # HL for current AI / GT comparison set
            hl_ai_stat, hl_ai_p, hl_ai_groups, _ = hosmer_lemeshow_test(y_true, ai_prob, g=10)
            hl_gt_stat_local, hl_gt_p_local, hl_gt_groups_local, _ = hosmer_lemeshow_test(y_true_gt, gt_prob, g=10)
            ...
            all_rows.append({
                ...
                "DeLong_p": delong_p,
                "HL_GT_stat": hl_gt_stat_local,
                "HL_GT_p": hl_gt_p_local,
                "HL_GT_groups": hl_gt_groups_local,
                "HL_AI_stat": hl_ai_stat,
                "HL_AI_p": hl_ai_p,
                "HL_AI_groups": hl_ai_groups,
                "ACC_AI": acc_ai,
                "SEN_AI": sen_ai,
                "SPE_AI": spe_ai,
                "TP": tp,
            ...
            print(
                f"thr={thr:.2f} | mode={mode_name:15s} | "
                f"AUC={auc_ai:.3f} | Zero={original_zero} | Rescue={rescued} | Impute={imputed} | "
                f"DeLong={delong_p:.4f} | HL_AI={hl_ai_p:.4f} | NI0.05={ni_005}"
            )
    ...
    if os.path.exists(final_patientwise):
        ...
        # Final HL
        hl_gt_stat_final, hl_gt_p_final, hl_gt_groups_final, hl_gt_table_final = hosmer_lemeshow_test(
            y_gt_final, gt_final_prob, g=10
        )
        hl_ai_stat_final, hl_ai_p_final, hl_ai_groups_final, hl_ai_table_final = hosmer_lemeshow_test(
            y_final, ai_final_prob, g=10
        )

        if len(hl_gt_table_final) > 0:
            hl_gt_table_final.to_csv(
                os.path.join(EVAL_DIR, f"Final_{FINAL_MODE_FOR_PLOTS}_{final_tag}_GT_HL_Table.csv"),
                index=False,
                encoding='utf-8-sig'
            )
        if len(hl_ai_table_final) > 0:
            hl_ai_table_final.to_csv(
                os.path.join(EVAL_DIR, f"Final_{FINAL_MODE_FOR_PLOTS}_{final_tag}_AI_HL_Table.csv"),
                index=False,
                encoding='utf-8-sig'
            )
        ...
        plot_final_calibration(
            ...
            hl_gt_p=hl_gt_p_final,
            hl_ai_p=hl_ai_p_final
        )
        ...
        with open(os.path.join(EVAL_DIR, "Hosmer_Lemeshow_Final.txt"), "w", encoding="utf-8") as f:
            f.write("Hosmer-Lemeshow test for final deployment setting\n")
            f.write("=================================================\n")
            f.write(f"Final threshold = {FINAL_THRESHOLD_FOR_PLOTS}\n")
            f.write(f"Final mode      = {FINAL_MODE_FOR_PLOTS}\n\n")
            f.write(f"GT contour : HL chi-square = {hl_gt_stat_final:.6f}, p = {hl_gt_p_final:.6f}, groups = {hl_gt_groups_final}\n")
            f.write(f"AI contour : HL chi-square = {hl_ai_stat_final:.6f}, p = {hl_ai_p_final:.6f}, groups = {hl_ai_groups_final}\n")

        print("\n📌 Final deployment HL:")
        print(f"   GT contour | HL = {hl_gt_stat_final:.3f} | P = {hl_gt_p_final:.4f} | groups = {hl_gt_groups_final}")
        print(f"   AI contour | HL = {hl_ai_stat_final:.3f} | P = {hl_ai_p_final:.4f} | groups = {hl_ai_groups_final}")
    ...
    with open(os.path.join(EVAL_DIR, "Step4_KeyFindings.txt"), "w", encoding="utf-8") as f:
        ...
        f.write(f"GT reference HL  = {hl_gt_stat_full:.4f}, p = {hl_gt_p_full:.4f}, groups = {hl_gt_groups_full}\n\n")
    ...
    print(f"📄 HL summary: {os.path.join(EVAL_DIR, 'Hosmer_Lemeshow_Final.txt')}")
